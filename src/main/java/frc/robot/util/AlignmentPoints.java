...
public class AlignmentPoints {
  ...
  public static Optional<Pose2d> getPoint(String category) {
    ...
    if (categoryName.toLowerCase().equals("closest")) {
      categoryName = getClosestCategory(subString[0], GlobalPosition.Get()).orElse(null);
    ...
    if (pointName.toLowerCase().equals("closest")) {
      pointName = getClosestPoint(subString[0], categoryName, GlobalPosition.Get()).orElse(null);
      if (pointName == null) {
        return Optional.empty();
      }
    }
